...
public class MTreeReducer extends MapReduceBase
    ...
    public void reduce(IntWritable key, Iterator<Text> value,
        ...
        while (value.hasNext()){
            Point point = new Point(value.next().toString(),0);
            points.add(point);
            for(int i = 0; i < point.get_dem(); i++){
                center.add_coord(i, points.get(points.size() - 1).get_coord()[i]);
            }
        }
        for(int i = 0; i< center.get_dem(); i++){
        ...
        for(int i = 0; i < points.size(); i++){
            output.collect(null, new Text(points.get(i).toString()));
        } 
